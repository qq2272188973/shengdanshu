...
const PhotoOrnaments = ({ state }: { state: 'CHAOS' | 'FORMED' }) => {
  ...
  useEffect(() => {
    ...
    const texList = Array.isArray(textures) ? textures : [textures];
  ...
  const data = useMemo(() => {
    return new Array(count).fill(0).map((_, i) => {
      ...
      const yJitter = (Math.random() - 0.5) * 0.35;
      const finalRadius = currentRadius + radialWobble;
      const finalTheta = theta + angularJitter;
      const targetPos = new THREE.Vector3(
        finalRadius * Math.cos(finalTheta),
        y + yJitter,
        finalRadius * Math.sin(finalTheta)
      );

      const isBig = Math.random() < 0.12;
      const baseScale = isBig ? 1.6 : 0.75 + Math.random() * 0.35;
...
const TreeTrunk = () => {
  ...
  return (
    <mesh
      ...
      position={[0, (-CONFIG.tree.height / 2) + (trunkHeight / 2) - 0.2, 0]}
      castShadow
      receiveShadow
    />
  );
...
const FallingSnow = () => {
  ...
  const { positions, speeds, drift } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      positions[ix + 1] = (Math.random() - 0.5) * 90;
      positions[ix + 2] = (Math.random() - 0.5) * 140;
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < speeds.length; i++) {
      const ix = i * 3;
      arr[ix + 1] -= speeds[i] * delta;
      arr[ix] += Math.sin(time * 0.35 + drift[i]) * delta * 0.35;
      arr[ix + 2] += Math.cos(time * 0.28 + drift[i]) * delta * 0.25;

      if (arr[ix + 1] < -45) {
        arr[ix + 1] = 45;
        arr[ix] = (Math.random() - 0.5) * 140;
        arr[ix + 2] = (Math.random() - 0.5) * 140;
      }
    }
